...
class CondAttribution:
    ...
    def __init__(self, concept: ChannelConcept | TransformerChannelConcept, model: torch.nn.Module,
                 device: torch.device = None, overwrite_data_grad=True, no_param_grad=True) -> None:
        """
    ...
    def _check_arguments(self, data, conditions, start_layer, exclude_parallel, init_rel):
        ...
        if self.overwrite_data_grad:
            ...
            warnings.warn(
                "'data' already has a filled .grad attribute. Set to None if not intended or set 'overwrite_grad' to True.")
    ...
    def __call__(
        ...
        if exclude_parallel:
            return self._conditions_wrapper(data, conditions, composite, record_layer, mask_map, start_layer, init_rel,
                                            on_device, True)
        else:
            return self._attribute(data, conditions, composite, record_layer, mask_map, start_layer, init_rel,
                                   on_device, False)
